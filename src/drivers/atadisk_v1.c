...
unsigned char ide_read(unsigned char channel, unsigned char reg) {
	...
	if   (reg < 0x08) result = _inb(channels[channel].base  + reg - 0x00);
	else if   (reg < 0x0C) result = _inb(channels[channel].base  + reg - 0x06);
	else if   (reg < 0x0E) result = _inb(channels[channel].ctrl  + reg - 0x0A);
	else if   (reg < 0x16) result = _inb(channels[channel].bmide + reg - 0x0E);
...
void ide_write(unsigned char channel, unsigned char reg, unsigned char data) {
	if   (reg > 0x07 && reg < 0x0C) ide_write(channel, ATA_REG_CONTROL, 0x80 | channels[channel].nIEN);
	if   (reg < 0x08) _outb(data, channels[channel].base  + reg - 0x00);
	else if   (reg < 0x0C) _outb(data, channels[channel].base  + reg - 0x06);
	else if   (reg < 0x0E) _outb(data, channels[channel].ctrl  + reg - 0x0A);
	else if   (reg < 0x16) _outb(data, channels[channel].bmide + reg - 0x0E);
	if   (reg > 0x07 && reg < 0x0C) ide_write(channel, ATA_REG_CONTROL, channels[channel].nIEN);
}
