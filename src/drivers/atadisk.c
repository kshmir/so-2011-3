...
static void sendComm(int ata, int rdwr, unsigned short sector){
	...
	_out(ata + WIN_REG6, 0xE0 | (ata << 4) | ((sector >> 24) & 0x0F));
...
static ssize_t read ( size_t _fd, char* buf, size_t count ){
	...
	for ( i=0; i<sectors; i++ ) {
		...
		if (!i){
			_disk_read(ata, buf, sector, offset, (offset+count>512)? size-offset : size);
		} else {
			_disk_read(ata, buf+(i*512)-offset, sector+i, 0, size);
		}
	}
...
static ssize_t write ( size_t _fd, char* buf, size_t bytes ){
	...
	for ( i=0; i<sectors; i++ ) {
		...
		if (!i) {
			_disk_write(ata, buf, size, sector, offset);
		} else {
			_disk_write(ata, buf+(i*512)-offset, size, sector+i, 0);
		}
